...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let userLogged = AuthManager.shared.userLogged {
            if BiometryAuthManager.shared.canUseBiometry {
                ...
                StorageManager.shared.getUser(by: userLogged) { regUser in
                    window.rootViewController = CoreTabVC(userLogged: regUser)
                    }
            }
        } else {
            let logViewModel = LogViewModel()
            window.rootViewController = LoginVC(viewModel: logViewModel)
        }
